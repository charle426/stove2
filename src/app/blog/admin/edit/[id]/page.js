...
export default function Edit() {
  ...
  function handleFileChange(e) {
    ...
    uploadBytes(imgRef, files?.[0])
      ...
      async function setSingleFeaturedBlog() {
          const snapshot = await getDocs(blogsCollection);
          const featuredBlogId = id;
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    const { title, content, file, isFeatured } = edited
    ...
    await updateDoc(docRef, formData)
      .then(() => {
        if(isFeatured) setSingleFeaturedBlog()
   ...
   <div className="flex flex-col md:flex-row">
        ...
        <div>
          <input
            ...
            className="bg-gray-200 rounded-[10px] w-full p-2"
          />
        </div>
        ...
        <div className="flex items-center gap-3 mb-5">
          <label htmlFor="checkbox" className="relative h-[3em] w-[3em] rounded-[1.2em] bg-gray-300 shadow-[inset_-1px_1px_4px_0px_#f0fffe,inset_1px_-1px_4px_0px_#00bdb0,-1px_2px_4px_0px_#00bdb0]">
            <input type="checkbox" checked={edited.isFeatured} onChange={handleChange} name="isFeatured" id="checkbox" className="peer appearance-none" />
        ...
        {wait ? (
          <button
            className="button ring-1 pointer-events-none select-none animate-pulse flex justify-center items-center max-w-[150px] py-2"
          >
            Please Wait...
          </button>
        ) : (
          <button
            type="submit"
            className="button  ring-1 ring-green-500 max-w-[150px] py-2"
          >
            update
          </button>
        )}
